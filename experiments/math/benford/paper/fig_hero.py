"""
Hero figure: L1 distance to Benford for three regimes.

Single panel, three curves, direct labels, no legend box.
  - addition only (flat: no convergence)
  - alternating add/mult (slow algebraic decay)
  - mixed BS(1,2) (exponential convergence)

Run: sage -python fig_hero.py
"""
...
def main():
    ...
    ax.semilogy(s_add, l1_add, color=C_ADD, linewidth=1.4, zorder=3)
    ax.semilogy(s_alt, l1_alt, color=C_ALT, linewidth=1.4, zorder=3)
    ax.semilogy(s_bs, l1_bs, color=C_BS, linewidth=1.8, zorder=4)

    # Noise floor
    ax.axhline(noise_floor, color='#999999', linestyle=':', linewidth=0.8,
               zorder=1)
    ax.text(15, noise_floor * 0.72, 'sampling floor',
            fontsize=7, color='#888888', ha='left', va='top')

    # Direct curve labels — placed where each curve is readable
    # Addition: flat at top, label inside the axes at right
    ax.text(760, l1_add[-1] * 1.15, 'addition only',
            fontsize=9, color=C_ADD, va='bottom', ha='left',
            fontweight='medium')

    # Alternating: label in the mid-descent region
    alt_y = l1_alt[np.argmin(np.abs(s_alt - 350))]
    ax.text(360, alt_y * 1.5, 'alternating add/mult',
            fontsize=9, color=C_ALT, va='bottom', ha='left',
            fontweight='medium')

    # BS(1,2): label in the early steep descent
    bs_y = l1_bs[np.argmin(np.abs(s_bs - 55))]
    ax.text(68, bs_y * 0.45, 'mixed (BS(1,2))',
            fontsize=9, color=C_BS, va='top', ha='left',
            fontweight='medium')
    ...
    out = os.path.join(HERE, 'fig_hero.png')
