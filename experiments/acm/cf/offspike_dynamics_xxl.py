...
B = 10
MAX_PQ = 2000
LOG10_SPIKE = 3.0          # match cf_khinchin_probe_xxl.py
PREC_BITS_LO = 1_500_000
PREC_BITS_HI = 3_000_000
K_PRIMES = 100_000
HERE = os.path.dirname(os.path.abspath(__file__))
OUT_PREFIX = 'offspike_dynamics_xxl'
...
def main():
    ...
    csv_path = os.path.join(HERE, f'{OUT_PREFIX}.csv')
    with open(csv_path, 'w', newline='') as f:
        csv.writer(f).writerows(csv_rows)
    print(f'\nwrote {csv_path}')

    spike_csv_path = os.path.join(HERE, f'{OUT_PREFIX}_spikes.csv')
    ...
    # Properly weighted pooled mean = Σ sum_small / Σ M_small.
    total_M = sum(row['M_small'] for row in summary)
    total_sum = sum(row['sum_small'] for row in summary)
    if total_M > 0:
        pooled = total_sum / total_M
        # GK conditional on a ≤ 10^LOG10_SPIKE.
        # Tail: contribution to E[log_b a] from a ≥ 10^LOG10_SPIKE.
        cutoff_a = 10 ** LOG10_SPIKE
        tail_E = 0.0
        tail_P = 0.0
        log_b = math.log(B)
        # Sum truncated at large k; tail of GK falls as ~1/k^2.
        for k in range(int(cutoff_a) + 1, 10 ** 6 + 1):
            p = log2(1.0 + 1.0 / (k * (k + 2)))
            tail_E += (math.log(k) / log_b) * p
            tail_P += p
        m1_cond = (m1 - tail_E) / (1.0 - tail_P)
        out.append(
            f'  pooled mean_small = {pooled:.6f}  '
            f'(GK uncond {m1:.6f}, GK cond a≤10^{LOG10_SPIKE:.0f} '
            f'{m1_cond:.6f})'
        )

    # Sub-canonical excess test.
    out.append('')
    out.append('## Test C: sub-canonical spike rate vs Khinchin renewal')
    out.append('##   M_sub_total = sum of sub-canonical (a > 10^3, not')
    out.append('##                 at canonical L) over all interior PQs')
    out.append('##   Khinchin-expected = (M_small + M_sub) · log2(1 + 1/(10^3 + 1))')
    M_sub_total = sum(row['M_sub'] for row in summary)
    interior_total = total_M + M_sub_total
    p_tail = log2(1.0 + 1.0 / (10 ** LOG10_SPIKE + 1))
    expected_sub = interior_total * p_tail
    var_sub = expected_sub * (1.0 - p_tail)
    sd_sub = math.sqrt(var_sub) if var_sub > 0 else 0.0
    z_sub = ((M_sub_total - expected_sub) / sd_sub) if sd_sub > 0 else float('nan')
    out.append(f'  interior PQs (small + sub): {interior_total}')
    out.append(f'  M_sub observed            : {M_sub_total}')
    out.append(f'  Khinchin expected         : {expected_sub:.4f}')
    out.append(f'  z (Poisson)               : {z_sub:+.4f}')

    summary_path = os.path.join(HERE, f'{OUT_PREFIX}_summary.txt')
